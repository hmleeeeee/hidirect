...
var UI = (function(){
	...
	, quick ={
		...
		, handler : function(){
			//1. .floating: 항상 떠있는 Case
			//2. floating_bottom: btn-footer-area fixed Case
			//3. floating_top: top 버튼 나오는 Case
		...
		handler : function($target, _openNum){
			$target.find('> .item').each(function(i){
				...
				function open(){
					...
					$head.find('.hidden').text('내용닫기');
					$head.attr('aria-expanded', true)
				}
				function close(){
					_this.removeClass('on');
					$head.attr('title','내용보기');
					$head.find('.hidden').text('내용보기');
				...
				$head.on('click', function(){
					...
					$body.stop().slideToggle(160, function(){
						//유의사항 scroll Top
						var posY = $(this).closest('.accordion').offset().top;
						if($(this).closest('.accordion').hasClass('notice')){
							if(!!$(this).closest('.modal_body').length){
								// console.log('모달')
								$(this).closest('.wrapper').scrollTop(posY);
								// console.log('유의사항',posY)
							}else if(!!$(this).closest('#content').length){
								// console.log('바닥페이지')
					...
					_this.siblings().find('.fold_header > a').attr('title','내용보기');
					_this.siblings().find('.fold_header > a').find('.hidden').text('내용보기');
	...
	, notice = {
		//유의사항 및 배너 하단에 고정 Case
		//[3/28] : 수정예정, 모달케이스 및 화면 튐현상, margin 및 footer 16
		setPos : function(){
			var $sectionFix = jMap.contentWrap.find('> .section.bottom_fix'); 

			$sectionFix.removeClass('show');

			if(!!$sectionFix.length){
				setTimeout(function(){
					jMap.contentWrap.addClass('bottom_fix');
					var winH = $(window).height();
					...
					var contentPT = Number(jMap.contentWrap.css('padding-top').replace('px', ''));//.content_wrap padding-top
					var contentPB = (!!jMap.contentFooter.length) ? conFooterH : 64;
					
					var sectionFixH = $sectionFix.outerHeight(true);
					var sectionH = 0;
					var mt = 0;

					//.bottom_fix 외 전체 section 높이
					jMap.contentWrap.find('> .section').each(function(){
						if(!!!$(this).hasClass('bottom_fix')){
							sectionH += $(this).outerHeight(true);
						}
					});

					// console.log('aa:',contentWrapH , sectionH, sectionFixH);
					// console.log('aa:',contentWrapH -(sectionH + sectionFixH));
					if((sectionH + sectionFixH) < contentWrapH){
						//1. footer fix 없을때 badding-bottom 간격: 64px
						//2. footer fix 있을때 간격: 24px =  (footer padding-top: 16px) + 8
						//40px = section padding-top
						if(!!jMap.contentFooter.length) {
							mt = (contentWrapH - sectionH - sectionFixH) - contentPT - 8 + 40; 
							contentPB = contentPB + 8;
						}else{
							mt = (contentWrapH - sectionH - sectionFixH) - contentPT - contentPB + 40;
						}
					}

					if(mt<32) mt=32;

					$sectionFix.css({'margin-top': mt });
					$sectionFix.css({'padding-bottom': contentPB}); 
					$sectionFix.addClass('show');
				},100)
			}
		}
	}
...
var layerPop = (function($){
	...
	, init = function(layer){
		...
		// var conFooterH = (!!$footer.length) ? $footer.outerHeight() : 0;
		// var footerH = (!!$footer.length) ? footerH : 64;//no_footer
		...
		function setHeight(){
			...
			if(!!$sectionFix.length){
				...
				$bodyWrap.addClass('bottom_fix');
				var contentPT = Number($bodyWrap.css('padding-top').replace('px', '')); //content_wrap 
				var contentMB = Number($bodyWrap.css('margin-bottom').replace('px', '')); //content_wrap 
				var contentPB = (!!$footer.length) ? 0 : 64;
				...
				//.bottom_fix 외 전체 section 높이
				$bodyWrap.find('> .section').each(function(){
					if(!!!$(this).hasClass('bottom_fix')){
						sectionH += $(this).outerHeight(true);
					}
				});

				// console.log(contentWrapH -(sectionH + sectionFixH));
				if((sectionH + sectionFixH) < contentWrapH){
					//1. footer fix 없을때 badding-bottom 간격: 64px
					//2. footer fix 있을때 간격: 24px;
					if(!!$footer.length) {
						mt = (contentWrapH - sectionH - sectionFixH) - contentPT - 24; 
						contentPB = contentPB + 24;
					}else{
						mt = (contentWrapH - sectionH - sectionFixH) - contentPT - contentPB;
		...
		$(window).resize(function(){
			setTimeout(function(){setHeight();}, 10);//[3/28] 모달에서 사이즈 초기 높이 못 잡는 버그 대응
		}).resize();


		// if(!!!$layer.hasClass('alert')){
		// 	$bodyWrap.css({'height': 'calc(100vh - '+bodyWrapH+')'});	
		// }
	}
